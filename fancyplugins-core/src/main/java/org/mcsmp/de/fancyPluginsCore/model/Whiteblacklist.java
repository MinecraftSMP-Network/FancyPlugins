...
public final class Whiteblacklist {
	...
	/**
	 * Return true if {@link ValidCore#isInList(String, Iterable)} returns true
	...
	public boolean isInList(final String item) {
		...
		final boolean match = ValidCore.isInList(item, this.items);
	...
	public boolean isInListRegex(final String item) {
		...
		final boolean match = this.compileAsPatterns ? ValidCore.isInListRegexFast(item, this.patterns) : ValidCore.isInListRegex(item, this.items);

		return this.whitelist ? match : !match;
	}

	/**
	 * Return true if {@link ValidCore#isInListStartsWith(String, Iterable)} returns true
	 * inverting it according to the {@link #isWhitelist()} flag.
	 *
	 * @param item
	 * @return
	...
	public boolean isInListStartsWith(final String item) {
		...
		final boolean match = ValidCore.isInListStartsWith(item, this.items);

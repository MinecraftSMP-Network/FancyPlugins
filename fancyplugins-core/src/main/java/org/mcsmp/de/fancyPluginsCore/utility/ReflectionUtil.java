...
public final class ReflectionUtil {
	 ...
	 */
	private static final Map<Class<? extends Enum<?>>, Map<String, MinecraftVersion.V>> legacyEnumTypes = new HashMap<>();
	 ...
	 * @param map
	 */
	public static void addLegacyEnumType(final Class<? extends Enum<?>> enumClass, final Map<String, MinecraftVersion.V> map) {
	...
	public static <E> E lookupEnum(final Class<E> typeOf, String name, final String errorMessage) {
		...
		if (result == null) {
			...
			final Map<String, MinecraftVersion.V> legacyMap = legacyEnumTypes.get(typeOf);

			if (legacyMap != null) {
				final MinecraftVersion.V since = legacyMap.get(name);
	 ...
	 */
	@SneakyThrows
	public static <T> TreeSet<Class<T>> getClasses(@NonNull final MinecraftPlugin plugin, final Class<T> extendingClass) {
